...
- **A manager that is scheduled automatically never resumes — it must *acquire*.** Resume detection
  looks for a previous **finished** job with the same `name` *and* the same flow id, and a flow id
  only survives across jobs when it is given explicitly (`--flow-id`) or inherited from the job's own
  tags (i.e. a clone). A manager started fresh by a scheduler gets an autogenerated one, so it begins
  with an empty view and will schedule children **alongside** any a previous instance left running.
  Set `acquire_all_jobs = True` so it adopts them instead: owned children are then matched by
  `PARENT_NAME=<name>` alone. If several instances of the manager share a name but target different
  spiders, also override `get_owned_jobs()` to filter by the target spider, or one instance will
