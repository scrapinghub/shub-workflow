...
class AcquireAllTestManager(CrawlManager):

    name = "test"
    acquire_all_jobs = True
...
class CrawlManagerTest(TestCase):
    ...
    @patch("shub_workflow.crawl.WorkFlowManager.schedule_spider")
    def test_check_resume_workflow_filters_by_flow_id(
        self, mocked_super_schedule_spider, mocked_add_job_tags, mocked_get_jobs
    ):
        """The resume candidate is searched by flow id, and children jobs (tagged with PARENT_NAME
        instead of NAME) sharing that same flow id are not taken as the resumed job."""
        with script_args(["myspider", "--flow-id=3a20"]):
            manager = TestManager()

        mocked_get_jobs.side_effect = [
            # same flow id: a children job first, then the actual resumed job
            [
                {"tags": ["FLOW_ID=3a20", "PARENT_NAME=test"], "key": "999/1/1"},
                {"tags": ["FLOW_ID=3a20", "NAME=test", "OTHER=other"], "key": "999/10/1"},
            ],
            # the owned running jobs
            [],
            # the owned finished jobs
            [],
        ]
        ...
        self.assertTrue(manager.is_resumed)
        self.assertEqual(mocked_get_jobs.call_args_list[0][1]["has_tag"], ["FLOW_ID=3a20"])
        mocked_add_job_tags.assert_any_call(tags=["FLOW_ID=3a20", "NAME=test", "OTHER=other"])

    @patch("shub_workflow.crawl.WorkFlowManager.schedule_spider")
    def test_autogenerated_flow_id_doesnt_check_resume(
        self, mocked_super_schedule_spider, mocked_add_job_tags, mocked_get_jobs
    ):
        """An autogenerated flow id can never match a previous job, so the resume check must be
        skipped. Running jobs are still acquired when acquire_all_jobs is set."""
        with script_args(["myspider"]):
            manager = AcquireAllTestManager()

        mocked_get_jobs.side_effect = [
            # the owned running job, scheduled by a previous instance of this same manager
            [{"spider": "myspider", "key": "999/1/1", "tags": ["FLOW_ID=other", "PARENT_NAME=test"]}],
            # the owned finished jobs
            [],
        ]
        manager._on_start()
        ...
        self.assertFalse(manager.is_resumed)
        self.assertEqual(len(manager._running_job_keys), 1)
        # only resume_workflow() hit the api: no scan for a resume candidate
        self.assertEqual(mocked_get_jobs.call_count, 2)
        for call in mocked_get_jobs.call_args_list:
            self.assertNotIn("has_tag", call[1])

        # the acquired job occupies the single available slot, so no new job is scheduled
        manager.is_finished = lambda jobkey: None
        result = next(manager._run_loops())
        self.assertTrue(result)
        self.assertFalse(mocked_super_schedule_spider.called)
