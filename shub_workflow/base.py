...
class WorkFlowManager(BaseLoopScript, WorkFlowManagerProtocol):
    ...
    def _check_resume_workflow(self):
        kwargs = {}
        if metadata := self.get_job_metadata():
            # only a previous job of this same script can be the one being resumed.
            kwargs["spider"] = self.get_metadata_key(metadata, "spider")
        # FLOW_ID is the selective tag, so it is the one used for filtering server side: filtering by
        # NAME instead means scanning every job ever created by any instance of this same script.
        # Only one has_tag filter is honored server side (several ones are ORed), so NAME is checked
        # here. Children jobs are tagged with PARENT_NAME rather than NAME, so they are discarded.
        for job in self.get_jobs(state=["finished"], meta=["tags"], has_tag=[f"FLOW_ID={self.flow_id}"], **kwargs):
            if f"NAME={self.name}" in job["tags"]:
                inherited_tags = []
                for tag in job["tags"]:
                    if len(tag.split("=")) == 2:
    ...
    def _on_start(self):
        # A resume can only be detected when the flow id was explicitly provided, that is, when the
        # job was manually scheduled as a continuation of a previous one (typically, a clone). An
        # autogenerated flow id can never match a previous job, so checking it is only a waste of
        # api calls. Job acquisition on automatic scheduling is what acquire_all_jobs is for.
